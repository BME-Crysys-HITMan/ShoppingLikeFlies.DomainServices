...
namespace ShoppingLikeFiles.DomainServices.Mappings
{
    ...
    {
        public static IMapperConfigurationExpression ConfigureCaffToTag(this IMapperConfigurationExpression mapperConfigurationExpression)
        {
            mapperConfigurationExpression.CreateMap<CaffToTag, CaffTagDTO>()
                .ForMember(dest => dest.Id, m => m.MapFrom(src => src.CaffTagId))
                .ForMember(dest => dest.Tag, m => m.MapFrom(src => src.CaffTag.Id));

            mapperConfigurationExpression.CreateMap<CaffToTag, CaffDTO>()
